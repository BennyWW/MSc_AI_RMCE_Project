            ...
            SetUp                 :       [block, block, block, block,...]
                block                   :       [Features, ObjectiveScores]
                    Features            :       [numberOfWalls, numberOfSecrets, numberOfTrees, centralHex, allyHome]
                        ...
                        spawns_Ally     :           * binary [000]
                        spawns_Enemy    :           * binary [000]
...
class SetUp:
    ...
    def __len__(self):
        ...
        - The map is a 40 X 40 grid of blocks. That means there are a total of 1600 blocks and since each block has 22
            indices, the length of the binary list will be 1600 * 22 = 35,200 indices
        """
    ...
    def Mutate(self, individual):
        ...
        """
        [000000       0000        0000        0         0       000     000] [000000       0000        0000        0         0       000     000]
        [walls        secrets     trees   CentralHex  allyHome  Allies  Enemies]
        """
    ...
    def Fitness(self, setUpList, OCEAN_Levels, printResults, saveFile):
        ...
        if printResults:
            print(f'walls:      {sum(walls)} / {len(walls)}\n'
        ...
        #  Shooter:        Sum of [Enemies, Walls, CentralHex]
        OCEAN_setUp['C'] = sum(walls + centralHexes + enemySpawns)
        OCEAN_setUp_Max['C'] = len(walls + centralHexes + enemySpawns)
        #  Violence:       Sum of [dangerous enemies]
        ...
        OCEAN_setUp['N'] = sum(trees + grass + interactableObjs)
        OCEAN_setUp_Max['N'] = len(trees + grass + interactableObjs)

        if printResults:
            print(
                f"Set-up O Score: {OCEAN_setUp['O']}\n"
                f"Set-up C Score: {OCEAN_setUp['C']}\n"
                f"Set-up E Score: {OCEAN_setUp['E']}\n"
                f"Set-up A Score: {OCEAN_setUp['A']}\n"
                f"Set-up N Score: {OCEAN_setUp['N']}\n"
            )
        ...
        if printResults:
            print(
                f"Set-up O level: {OCEAN_setUp_Trait_levels['O']}%\n"
                f"Set-up C level: {OCEAN_setUp_Trait_levels['C']}%\n"
                f"Set-up E level: {OCEAN_setUp_Trait_levels['E']}%\n"
                f"Set-up A level: {OCEAN_setUp_Trait_levels['A']}%\n"
                f"Set-up N level: {OCEAN_setUp_Trait_levels['N']}%\n"
            )
            print(
        ...
        if saveFile:
            with open(
                    'E:/Unity/Projects/Builds/PC/MSc_AI_RMCE_PRoject/MSc_AI_RMCE__Project_Data/StreamingAssets/Genetic Algorithm Results/Set Up Result.csv',
                    mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerows(self.GetBlocks(setUpList))

            # with open('E:/Unity/Projects/Builds/PC/MSc_AI_RMCE_PRoject/MSc_AI_RMCE__Project_Data/StreamingAssets/Genetic Algorithm Results/Set Up Result.txt',
            #           "w") as file:
            #     # Iterate through the nested list and write each sublist as a line in the file
            #     for sublist in self.GetBlocks(setUpList):
            #         line = "".join(map(str, sublist))  # Join elements
            #         print(line)
...
def GeneticAlgorithm(OCEAN_Levels, population_size, generations, pCrossover, pMutation, m_IndMultiplier):
    ...
    return setUp.Fitness(best, OCEAN_Levels, False, False)
    # endregion
...
def Main():
    ...
    OCEAN_levels_dummy = {"O": 46, "C": 89, "E": 34, "A": 73, "N": 45}

    print('Cluster 1')
    # GeneticAlgorithm(OCEAN_levels_C1, 50, 20, .9, .5, 0.7)
    print('Cluster 2')
    # GeneticAlgorithm(OCEAN_levels_C2, 40, 50, .45, .65, 0.4)
    print('Cluster 3')
    # GeneticAlgorithm(OCEAN_levels_C3, 40, 30, .7, .8, 0.5)
    print('Cluster 4')
    GeneticAlgorithm(OCEAN_levels_C4, 40, 30, .7, .8, 0.5)
    # GeneticAlgorithm(OCEAN_levels_dummy, 50, 40, .9, .9, 0.5)
    return
