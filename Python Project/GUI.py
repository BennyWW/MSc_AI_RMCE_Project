...
def UI():
    ...
    root = tk.Tk()
    root.title("Big Five Personality (O.C.E.A.N) Test")
    ...
    personality_frame.pack(padx=0, pady=10, fill=tk.BOTH, expand=True)

    # Create a canvas for the personality test content
    canvas = tk.Canvas(personality_frame, bg="white")
    ...
    for trait, question_list in questions.items():
        ttk.Label(test_content, text=f"{trait} Trait:", foreground="black", background="white").grid(row=row, column=0,
                                                                                                     sticky="w",
                                                                                                     padx=10,
                                                                                                     pady=5)
        row += 1
        for i, question in enumerate(question_list):
            ttk.Label(test_content, text=f"{i + 1}. {question}", foreground="black", background="white").grid(row=row,
                                                                                                              column=0,
                                                                                                              columnspan=2,
                                                                                                              sticky="w",
                                                                                                              padx=10)
            ttk.Radiobutton(test_content, text="Strongly Disagree", variable=responses[trait], value=str(1),
                            style="").grid(row=row, column=2)
            ttk.Radiobutton(test_content, text="Disagree", variable=responses[trait], value=str(2),
                            style="").grid(
                row=row, column=3)
            ttk.Radiobutton(test_content, text="Neutral", variable=responses[trait], value=str(3),
                            style="").grid(
                row=row, column=4)
            ttk.Radiobutton(test_content, text="Agree", variable=responses[trait], value=str(4), style="").grid(
                row=row, column=5)
            ttk.Radiobutton(test_content, text="Strongly Agree", variable=responses[trait], value=str(5),
                            style="").grid(row=row, column=6)
            row += 1

    # Submit Button
    submit_button = ttk.Button(test_content, text="Submit", command=submit_personality_test, style="TButton")
